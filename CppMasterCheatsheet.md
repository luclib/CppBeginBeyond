...
### Deep Copying
**Deep Copying** creates a **copy** of the pointed-to data; each copy will hvae a pointer to a *unique* storage location in the heap.
...
The same aplies to objects as well. When making calling copy constructors over and over again, we can create a *lot* of overhead leading to a significant *performance bottleneck*.

To address this, C++11 introduced move semantics and the **move constructor** 
* The **Move constructor** moves an object rathers than copy it.
* Optional but *recommended* when using raw pointers
* Some compilers may optimize copying away completely via *copy elision*
  * **Copy elision** is a compiler optimization technique that eliminates unnecessary copying via **RVO-Return Value Optimization**


**r-value references** are refrences to r-value references, that is temporary objects.
* Used by move constructor and move assignment operator to efficiently move an object rather than copy it.
* R-value reference operator: **&&**

> **NOTE**: An l-value reference is a reference that has an address to variable and a name.
